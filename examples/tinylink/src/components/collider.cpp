...
bool Collider::get_cell(int x, int y) const {
  ...
  BLAH_ASSERT(x >= 0 && y >= 0 && x < m_grid.columns && y < m_grid.rows,
...
Collider *Collider::first(uint32_t mask, Point offset) {
  if (world()) {
    ...
    while (other) {
      if (other != this && (other->mask & mask) == mask &&
...
void Collider::render() {
  // pti_rect(m_rect.x + entity()->position.x, m_rect.y + entity()->position.y,
  // 16,
  //          16, 0xFF0000FF);
  ...
  if (m_shape == Shape::Rect) {
    ...
    for (int x = 0; x < m_grid.columns; x++) {
      for (int y = 0; y < m_grid.rows; y++) {
        ...
        // batch.rect_line(Rect(x * m_grid.tile_size, y * m_grid.tile_size,
        //                      m_grid.tile_size, m_grid.tile_size),
        //                 1, color);
        pti_rect(x * m_grid.tile_size, y * m_grid.tile_size, m_grid.tile_size,
                 m_grid.tile_size, 0xFF0000FF);
      }
    }
  }
}

bool TL::Collider::rect_to_rect(const Collider *a, const Collider *b,
                                Point offset) {
...
bool TL::Collider::rect_to_grid(const Collider *a, const Collider *b,
                                Point offset) {
  // get a relative rectangle to the grid
  Recti rect =
      a->m_rect + a->entity()->position + offset - b->entity()->position;

  // get the cells the rectangle overlaps
  int left = Calc::clamp((int)Calc::floor(rect.x / (float)b->m_grid.tile_size),
                         0, b->m_grid.columns);
  int right =
      Calc::clamp((int)Calc::ceiling(rect.right() / (float)b->m_grid.tile_size),
                  0, b->m_grid.columns);
  int top = Calc::clamp((int)Calc::floor(rect.y / (float)b->m_grid.tile_size),
                        0, b->m_grid.rows);
  int bottom = Calc::clamp(
      (int)Calc::ceiling(rect.bottom() / (float)b->m_grid.tile_size), 0,
      b->m_grid.rows);
