...
void Player::update() {
  ...
  // Sprite Stuff
  {
    ...
    anim->scale =
        Vec2f::approach(anim->scale, Vec2f(m_facing, 1.0f), Time::delta * 4);
  ...
  // NORMAL STATE
    ...
    // Horizontal Movement
    {
      // Acceleration
      mover->speed.x +=
          input * (m_on_ground ? ground_accel : air_accel) * Time::delta;

      // Maxspeed
      auto maxspd = (m_on_ground ? max_ground_speed : max_air_speed);
      if (Calc::abs(mover->speed.x) > maxspd) {
        mover->speed.x =
            Calc::approach(mover->speed.x, Calc::sign(mover->speed.x) * maxspd,
                           2000 * Time::delta);
      }
      ...
      if (input == 0 && m_on_ground)
        mover->speed.x =
            Calc::approach(mover->speed.x, 0, friction * Time::delta);
    ...
    // Invoke Jumping
    {
      if (pti_pressed(PTI_A) && mover->on_ground()) {
  ...
  // HURT STATE
    ...
    mover->speed.x =
        Calc::approach(mover->speed.x, 0, hurt_friction * Time::delta);
  }
  ...
  if (!m_on_ground) {
    float grav = gravity;
    if (m_state == st_normal && Calc::abs(mover->speed.y) < 20 &&
        pti_down(PTI_A))
      grav *= 0.4f;
