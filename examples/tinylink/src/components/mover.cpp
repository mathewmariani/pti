  ...
  if (collider) {
    ...
    while (amount != 0) {
      ...
      if (!hit_something && sign > 0)
        hit_something = collider->check(Mask::jumpthru, Point(0, sign)) &&
                        !collider->check(Mask::jumpthru, Point(0, 0));
...
bool Mover::on_ground(int dist) const {
  ...
  return
      ...
      // jumpthru
      (collider->check(Mask::jumpthru, Point(0, dist)) &&
       !collider->check(Mask::jumpthru, Point(0, 0)));
}
...
void Mover::update() {
  ...
  Vec2f total = m_remainder + speed * Time::delta;
