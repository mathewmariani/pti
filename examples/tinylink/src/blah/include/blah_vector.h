...
template <class T> inline Vector<T>::Vector(int capacity) {
  m_buffer = nullptr;
  m_count = m_capacity = 0;
  reserve(capacity);
}

template <class T> inline Vector<T>::Vector(const Vector &src) {
  m_buffer = nullptr;
  m_count = m_capacity = 0;
...
template <class T> inline Vector<T>::Vector(Vector &&src) noexcept {
...
template <class T> inline Vector<T>::Vector(const InitializerList<T> &list) {
  m_buffer = nullptr;
  m_count = m_capacity = 0;
...
template <class T> inline Vector<T>::~Vector() { dispose(); }

template <class T> inline Vector<T> &Vector<T>::operator=(const Vector &src) {
  clear();
  reserve(src.m_capacity);
  for (int i = 0; i < src.m_count; i++)
    new (m_buffer + i) T(src.m_buffer[i]);
...
template <class T> inline void Vector<T>::dispose() {
  clear();
...
template <class T> inline int Vector<T>::size() const { return m_count; }

template <class T> inline int Vector<T>::capacity() const { return m_capacity; }

template <class T> inline void Vector<T>::reserve(int cap) {
  if (cap > m_capacity) {
...
template <class T> inline void Vector<T>::resize(int new_count) {
...
template <class T> inline T *Vector<T>::expand(int amount) {
  if (amount > 0) {
    int count = m_count;
...
template <class T> inline void Vector<T>::push_back(const T &item) {
  reserve(m_count + 1);
  new (m_buffer + m_count) T(item);
  m_count++;
}

template <class T> inline void Vector<T>::push_back(T &&item) {
  reserve(m_count + 1);
...
template <class T> inline T *Vector<T>::data() { return m_buffer; }

template <class T> inline const T *Vector<T>::data() const { return m_buffer; }

template <class T> inline T *Vector<T>::begin() { return m_buffer; }

template <class T> inline const T *Vector<T>::begin() const { return m_buffer; }

template <class T> inline T *Vector<T>::end() { return m_buffer + m_count; }

template <class T> inline const T *Vector<T>::end() const {
...
template <class T> inline void Vector<T>::erase(int index, int elements) {
...
template <class T> inline T Vector<T>::pop() {
  BLAH_ASSERT(m_count > 0, "Index out of range");
