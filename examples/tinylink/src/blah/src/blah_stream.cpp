#include "../include/blah_stream.h"
#include "../include/blah_string.h"
...
u8 Stream::read_u8(Endian endian) {
  u8 value;
  read_data(&value, sizeof(u8));
  if (!Calc::is_endian(endian))
    Calc::swap_endian(&value);
  return value;
}

u16 Stream::read_u16(Endian endian) {
  u16 value;
  read_data(&value, sizeof(u16));
  if (!Calc::is_endian(endian))
    Calc::swap_endian(&value);
  return value;
}
...
FileStream::FileStream(const FileRef &file) : m_file(file) {}

size_t FileStream::length() const { return (m_file ? m_file->length() : 0); }
...
bool FileStream::is_open() const { return m_file ? true : false; }
...
size_t MemoryStream::length() const { return m_length; }

size_t MemoryStream::position() const { return m_position; }

size_t MemoryStream::seek(size_t seek_to) {
  return m_position =
             (seek_to < 0 ? 0 : (seek_to > m_length ? m_length : seek_to));
}

size_t MemoryStream::read_data(void *ptr, size_t len) {
  const u8 *src = (m_data ? m_data : m_const_data);

  if (src == nullptr || ptr == nullptr || len <= 0 || m_length <= 0 ||
      m_position >= m_length)
    return 0;
...
size_t MemoryStream::write_data(const void *ptr, size_t len) {
  if (m_data == nullptr || ptr == nullptr || len <= 0 || m_length <= 0 ||
      m_position >= m_length)
    return 0;
...
BufferStream::BufferStream(int capacity) { m_buffer.resize(capacity); }

size_t BufferStream::length() const { return m_buffer.size(); }

size_t BufferStream::position() const { return m_position; }

size_t BufferStream::seek(size_t seek_to) {
  return m_position =
             (seek_to < 0
                  ? 0
                  : (seek_to > m_buffer.size() ? m_buffer.size() : seek_to));
}
...
void BufferStream::resize(size_t length) { m_buffer.resize(length); }
...
u8 *BufferStream::data() { return m_buffer.data(); }

const u8 *BufferStream::data() const { return m_buffer.data(); }
