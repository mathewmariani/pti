...
FileRef File::open(const FilePath &path, FileMode mode) {
  ...
  FileRef ref;
  Platform::file_open(path.cstr(), mode);
  if (ref)
    ref->m_mode = mode;
  return ref;
}
...
FileMode File::mode() const { return m_mode; }
...
void Directory::explore(const FilePath &path) { Platform::dir_explore(path); }
    ...
    // move up a directory
      ...
      if (normalized.length() > 0) {
        for (auto k = normalized.length() - 1; k > 0; k--)
          if (normalized[k - 1] == '/') {
            normalized =
                FilePath(normalized.begin(), normalized.begin() + k - 1);
            could_move_up = true;
            break;
          }
      }
