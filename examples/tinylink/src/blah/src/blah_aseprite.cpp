#include "../include/blah_aseprite.h"
#include "../include/blah_calc.h"
#include "../include/blah_filesystem.h"
...
Aseprite::Aseprite(Stream &stream) { parse(stream); }
...
void Aseprite::parse(Stream &stream) {
  ...
  // header
  {
    ...
    stream.read_u16(
        Endian::Little); // Number of colors (0 means 256 for old sprites)
...
void Aseprite::parse_layer(Stream &stream, int frame) {
  ...
  layer.visible =
      ((int)layer.flag & (int)LayerFlags::Visible) == (int)LayerFlags::Visible;
...
void Aseprite::parse_cel(Stream &stream, int frameIndex, size_t maxPosition) {
  ...
  if (celType == 0 || celType == 2) {
    ...
    // DEFLATE (zlib)
      ...
      // this could be optimized to use a buffer on the stack if we only read
      // set chunks at a time stbi's zlib doesn't have that functionality though
      ...
      int res = stbi_zlib_decode_buffer((char *)cel.image.pixels, olen, buffer,
                                        (int)size);
    ...
    if (mode == Modes::Grayscale) {
      ...
      for (int d = width * height - 1, s = (width * height - 1) * 2; d >= 0;
           d--, s -= 2)
...
void Aseprite::parse_tag(Stream &stream, int frame) {
  ...
  for (int t = 0; t < count; t++) {
    ...
    stream.seek(stream.position() + 8);
    tag.color = Color(stream.read_i8(), stream.read_i8(),
                      stream.read_i8(Endian::Little), 255);
    ...
    for (int dx = left, sx = -Calc::min(srcX, 0); dx < right; dx++, sx++) {
      for (int dy = top, sy = -Calc::min(srcY, 0); dy < bottom; dy++, sy++) {
        ...
        if (srcColor->a != 0) {
          ...
          dstColor->r = (unsigned char)(dstColor->r +
                                        (srcColor->r - dstColor->r) * sa / ra);
          dstColor->g = (unsigned char)(dstColor->g +
                                        (srcColor->g - dstColor->g) * sa / ra);
          dstColor->b = (unsigned char)(dstColor->b +
                                        (srcColor->b - dstColor->b) * sa / ra);
