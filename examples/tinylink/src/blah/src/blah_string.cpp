...
constexpr int blah_strlen(const char *cstr) {
  int len = 0;
  if (cstr)
    while (*(cstr + len) != '\0' && len < INT32_MAX)
      len++;
  return len;
}

constexpr bool blah_compare_ignore_case(char a, char b) {
...
#define BLAH_ASSERT_INPUT_STRING(input)                                        \
  BLAH_ASSERT(!(input >= cstr() && input < cstr() + length()),                 \
              "Assigning string to itself is bad news!")
} // namespace
...
void BaseString::append(const u16 *u16_cstr, const u16 *u16_cstr_end,
                        bool swap_endian) {
  ...
  while ((u16_cstr_end == nullptr && *u16_cstr != 0) ||
         (u16_cstr_end != nullptr && u16_cstr != u16_cstr_end)) {
    u16 next = (*u16_cstr++);
    if (swap_endian)
...
bool BaseString::starts_with(const char *cstr, bool ignore_case) const {
  ...
  auto compare =
      (ignore_case ? &blah_compare_ignore_case : &blah_compare_with_case);
...
Utf8::Utf8(const char *cstr) : str(cstr) {
