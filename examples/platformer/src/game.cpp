...
void game::load() {
	...
	for (i = 0; i < entity::EN_ROOM_COLS; i++) {
		for (j = 0; j < entity::EN_ROOM_ROWS; j++) {
			...
			switch (t) {
				case 48:
					entities.Create(std::make_unique<Player>(), XPOS(i), YPOS(j));
					pti_mset(tilemap, i, j, 0);
					break;
				case 49:
					entities.Create(std::make_unique<Coin>(), XPOS(i), YPOS(j));
					pti_mset(tilemap, i, j, 0);
					break;
				case 50:
					entities.Create(std::make_unique<Goomba>(), XPOS(i), YPOS(j));
					pti_mset(tilemap, i, j, 0);
					break;
				case 51:
					entities.Create(std::make_unique<Shooter>(), XPOS(i), YPOS(j));
					pti_mset(tilemap, i, j, 0);
					break;
			}
		}
