  ...
  for (int x = 0; x < columns; x++)
    for (int y = 0; y < rows; y++) {
      ...
      case 0xdf7126: {
        ...
        auto jumpthru_en =
            world.add_entity(offset + Point(x * tile_width, y * tile_height));
        auto jumpthru_col =
            jumpthru_en->add(Collider::make_rect(Recti(0, 0, 8, 4)));
      ...
      case 0x6abe30:
        if (!world.first<Player>())
          Factory::player(&world, world_position + (is_reload ? Point(0, -16)
                                                              : Point::zero));
        break;
...
void Game::update() {
  ...
  // Room Transition routine
    ...
    // increment ease
    m_next_ease =
        Calc::approach(m_next_ease, 1.0f, Time::delta / transition_duration);
...
void Game::render() {
  ...
  if (player) {
    ...
    for (int i = 0; i < Player::max_health; i++) {
      if (player->health >= i + 1) {
        ...
        n = full->frames[0].image.n;
        w = full->frames[0].image.w;
        h = full->frames[0].image.h;
        pti_sspr(n, pos.x, pos.y, w, h, false, false);
      } else {
        //   batch.tex(empty->frames[0].image, pos);
        n = empty->frames[0].image.n;
        w = empty->frames[0].image.w;
        h = empty->frames[0].image.h;
        pti_sspr(n, pos.x, pos.y, w, h, false, false);
      }
      pos.x += 12;
    }
  ...
  // {
  //   float scale =
  //       Calc::min(App::backbuffer()->width() / (float)buffer->width(),
  //                 App::backbuffer()->height() / (float)buffer->height());

  //   Vec2f screen_center =
  //       Vec2f(App::backbuffer()->width(), App::backbuffer()->height()) / 2;
  //   Vec2f buffer_center = Vec2f(buffer->width(), buffer->height()) / 2;

  //   App::backbuffer()->clear(Color::black);
  //   batch.push_matrix(Mat3x2f::create_transform(screen_center, buffer_center,
  //                                               Vec2f::one * scale, 0));
