#include "../include/blah_packer.h"
#include <algorithm>

using namespace Blah;

Packer::Packer()
    : max_size(8192), power_of_two(true), spacing(1), padding(1),
      m_dirty(false) {}

Packer::Packer(int max_size, int spacing, bool power_of_two)
    : max_size(max_size), power_of_two(power_of_two), spacing(spacing),
      padding(1), m_dirty(false) {}
...
void Packer::add(u64 id, const Image &image) {
  add_entry(id, image.width, image.height, image.pixels,
            Recti(0, 0, image.width, image.height));
}
...
void Packer::add(u64 id, const FilePath &path) { add(id, Image(path.cstr())); }
  ...
  if (right > left && bottom > top) {
    ...
    if (entry.packed.w == w && entry.packed.h == h) {
      ...
      for (int i = 0; i < entry.packed.h; i++)
        m_buffer.write((char *)(pixels + left + (top + i) * w),
                       sizeof(Color) * entry.packed.w);
    }
  }
...
const Vector<Packer::Entry> &Packer::entries() const { return m_entries; }
  ...
  if (count > 0) {
    ...
    Vector<Entry *> sources;
    {
      ...
      std::sort(sources.begin(), sources.end(),
                [](Packer::Entry *a, Packer::Entry *b) {
                  return a->packed.w * a->packed.h > b->packed.w * b->packed.h;
                });
    }

    // make sure the largest isn't too large
    if (sources[0]->packed.w + padding * 2 > max_size ||
        sources[0]->packed.h + padding * 2 > max_size) {
    ...
    while (packed < count) {
      ...
      int from = packed;
      int index = 0;
      Node *root = nodes[index++].reset(
          Recti(0, 0, sources[from]->packed.w + padding * 2 + spacing,
                sources[from]->packed.h + padding * 2 + spacing));
      ...
      while (packed < count) {
        ...
        if (node == nullptr) {
          bool canGrowDown =
              (w <= root->rect.w) && (root->rect.h + h < max_size);
          bool canGrowRight =
              (h <= root->rect.h) && (root->rect.w + w < max_size);
          bool shouldGrowRight =
              canGrowRight && (root->rect.h >= (root->rect.w + w));
          bool shouldGrowDown =
              canGrowDown && (root->rect.w >= (root->rect.h + h));

          if (canGrowDown || canGrowRight) {
            // grow right
            if (shouldGrowRight || (!shouldGrowDown && canGrowRight)) {
              Node *next = nodes[index++].reset(
                  Recti(0, 0, root->rect.w + w, root->rect.h));
              next->used = true;
              next->down = root;
              next->right = node =
                  nodes[index++].reset(Recti(root->rect.w, 0, w, root->rect.h));
              root = next;
            }
            // grow down
            else {
              Node *next = nodes[index++].reset(
                  Recti(0, 0, root->rect.w, root->rect.h + h));
              next->used = true;
              next->down = node =
                  nodes[index++].reset(Recti(0, root->rect.h, root->rect.w, h));
              next->right = root;
              root = next;
            }
          }
        ...
        // add
        node->used = true;
        node->down = nodes[index++].reset(Recti(
            node->rect.x, node->rect.y + h, node->rect.w, node->rect.h - h));
        node->right = nodes[index++].reset(
            Recti(node->rect.x + w, node->rect.y, node->rect.w - w, h));
            ...
            if (padding > 0) {
              ...
              for (int x = -padding; x < dst.w + padding; x++)
                for (int y = -padding; y < dst.h + padding; y++) {
                  ...
                  image.pixels[dst.x + x + (dst.y + y) * image.width] =
                      src[sx + sy * dst.w];
                }
            } else {
