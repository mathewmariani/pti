...
bool Image::save_png(Stream &stream) const {
  ...
  if (stream.is_writable()) {
    ...
    if (stbi_write_png_to_func(blah_stbi_write, &stream, width, height, 4,
                               pixels, width * 4) != 0)
      return true;
  }
...
bool Image::save_jpg(Stream &stream, int quality) const {
  ...
  if (stream.is_writable()) {
    if (stbi_write_jpg_to_func(blah_stbi_write, &stream, width, height, 4,
                               pixels, quality) != 0)
      return true;
  }
...
void Image::get_pixels(Color *dest, const Point &dest_pos,
                       const Point &dest_size, Recti source_rect) const {
  // can't be outside the source image
  if (source_rect.x < 0)
    source_rect.x = 0;
  if (source_rect.y < 0)
    source_rect.y = 0;
  if (source_rect.x + source_rect.w > width)
    source_rect.w = width - source_rect.x;
  if (source_rect.y + source_rect.h > height)
    source_rect.h = height - source_rect.y;
...
Image Image::get_sub_image(const Recti &source_rect) {
  ...
  get_pixels(img.pixels, Point::zero, Point(img.width, img.height),
             source_rect);
  return img;
}
